...
export async function UploadPfp(file) {
    ...
    const imagePath = `${process.env.SUPABSE_KEY}/storage/v1/object/public/avatars/${imageName}`
...
export async function UploadBanner(file) {
    const imageName = `${Math.random()}-${file[0].name}`.replaceAll("/", "")
    const imagePath = `${process.env.SUPABSE_KEY}/storage/v1/object/public/banners/${imageName}`
    const { error } = await supabase.storage.from("banners").upload(imageName, file[0])
    if (error) {
